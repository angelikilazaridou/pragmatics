...
local function eval_split(split, evalopt)
				...
				TP_FP_D = TP_FP_D + torch.sum(predicted2[i])
					...
					local s1 = properties[ii][{{i,i},{1,opt.vocab_size}}]:clone():apply(function(x)  if x > opt.threshold  then return 0 else return 1 end end)
					 ...
					 --feature wise precision, recall and F1
                                	...
                                	TP_FN = TP_FN + torch.sum(s2) --all relevant
                                	TP_FP = TP_FP + torch.sum(s1) -- all predictions
  ...
  local precision = TP/TP_FP
  local recall = TP/TP_FN
  ...
  return precision_D, recall_D, F1_D, precision, recall, F1
end

print(string.format('Params: vocab_size = %d',opt.vocab_size))
local  precision_D, recall_D, F1_D,  precision,recall,F1 = eval_split(opt.split, {num_images = opt.num_images})

print(string.format('Discriminativeness eval: Precision=%f Recall=%f F1=%f', precision_D, recall_D, F1_D))
print(string.format('Property eval: Precision=%f Recall=%f F1=%f', precision, recall, F1))
